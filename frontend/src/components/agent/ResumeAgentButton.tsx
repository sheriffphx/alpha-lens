...
export function ResumeAgentButton({ policyId }: ResumeAgentButtonProps) {
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      window.dispatchEvent(new Event("alphalens-policy-updated"));
    }
  }, [isSuccess]);
  ...
  return (
    <div className="flex items-center gap-3">
      <button
        onClick={resumeAgent}
        disabled={isPending || confirming || resumed || !isConnected}
      ...
      {error && (
        <span className="max-w-md text-xs text-red-300">{error.message}</span>
      )}
    </div>
  );
}
