...
export default function RiskPage() {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-500">Connect your wallet to analyse risk.</p>
        </div>
      </main>
    );
  }
  ...
  const concentrationRisk = largestAllocation >= 80 ? 100 : largestAllocation >= 60 ? 75 : largestAllocation >= 40 ? 50 : 0;
  const volatilityRisk = nativeAllocation >= 70 ? 100 : nativeAllocation >= 40 ? 60 : 0;
...
const topBestPools = [...pools]
  .sort((a, b) => (b.opportunityScore ?? b.yieldSanity.score) - (a.opportunityScore ?? a.yieldSanity.score))
  .slice(0, 10);

const topWorstPools = [...pools]
  .sort((a, b) => (a.opportunityScore ?? a.yieldSanity.score) - (b.opportunityScore ?? b.yieldSanity.score))
  .slice(0, 5);

  return (
    <main className="mx-auto min-h-screen max-w-7xl px-6 py-10">
      <header className="mb-8">
        <p className="font-mono text-xs tracking-[0.18em] text-[#00D4FF]">RISKS</p>
        <h1 className="mt-2 text-3xl font-semibold">Portfolio Risk</h1>
        <p className="mt-2 text-sm text-white/50">Exposure from wallet holdings and current BDEX pool conditions.</p>
      </header>

      <section className="rounded-xl border border-white/10 bg-white/[0.025] p-6">
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-start">
          <div>
            <p className="font-mono text-[11px] tracking-[0.12em] text-white/40">RISK SCORE</p>
          ...
          <p className="max-w-sm text-sm leading-6 text-white/55">
            This score evaluates concentration, native-asset volatility, and diversification from the balances currently visible in your wallet.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {resolvedPortfolioRisk.reasons.length > 0 ? (
            resolvedPortfolioRisk.reasons.map((reason) => (
              <div key={reason} className="flex gap-3 rounded-lg bg-white/[0.035] p-4">
            ...
            <div className="flex gap-3 rounded-lg bg-white/[0.035] p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-white/55">No concentration threshold has been triggered.</p>
              </div>
            </div>
          )}
            ...
            <div className="flex gap-3 rounded-lg bg-white/[0.035] p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-white/55">
                  {usd.format(portfolioAnalysis.idleCapitalUsd)} is currently held as stablecoins in your wallet and earning 0% APY.
                </p>
              </div>
            </div>
          )}
          ...
          {resolvedPortfolioRisk.warnings.map((warning) => (
            <div key={warning} className="flex gap-3 rounded-lg bg-amber-300/[0.04] p-4">
              <span className="text-amber-300">⚠</span>
              <div>
          ...
          <div className="flex gap-3 rounded-lg bg-white/[0.035] p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/55">
                No protocol positions are currently indexed. Contract exposure cannot be assessed until lending and LP positions are connected.
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-white/50">Scores use the same thresholds as your portfolio risk calculation.</p>
          <div className="mt-6 space-y-4">
            <RiskBar label="Liquidity" score={liquidityRisk} note="Not assessed" />
            <RiskBar label="Concentration" score={concentrationRisk} note={`${largestAllocation.toFixed(0)}% largest`} />
            <RiskBar label="Volatility" score={volatilityRisk} note={`${nativeAllocation.toFixed(0)}% native`} />
            <RiskBar label="Protocol" score={protocolRisk} note="No positions" />
          </div>
        </div>
      </section>
  ...
  <div>
    <div className="mb-4">
      <h2 className="text-xl font-semibold text-emerald-400">Top 10 High Opportunity Pools</h2>
      <p className="mt-1 text-sm text-white/50">BDEX pools with the highest overall opportunity scores.</p>
    </div>
    ...
    {poolsLoading ? (
      ...
      <div className="space-y-4">
        {topBestPools.map((pool, idx) => (
          <article key={pool.address} className="rounded-xl border border-emerald-500/20 bg-white/[0.025] p-5">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs text-emerald-400">#{idx + 1}</span>
                <div>
                  <h3 className="font-semibold">{pool.token0Symbol} / {pool.token1Symbol}</h3>
                  <p className="mt-1 text-sm text-white/50">TVL {usd.format(pool.tvl)} · Fee APR {pool.feeApr.toFixed(2)}%</p>
                </div>
              </div>
              <span className="w-fit rounded border border-emerald-400/25 bg-emerald-400/10 px-2.5 py-1 font-mono text-xs text-emerald-300">
                Score: {(pool.opportunityScore ?? pool.yieldSanity.score).toFixed(1)}
              </span>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <div className="rounded-lg bg-white/[0.04] p-3">
                <p className="text-xs text-white/45">Yield Quality</p>
                <p className="mt-1 font-mono text-sm">{pool.yieldSanity.score.toFixed(1)} / 100</p>
              </div>
              <div className="rounded-lg bg-white/[0.04] p-3">
                <p className="text-xs text-white/45">Liquidity Score</p>
                <p className="mt-1 font-mono text-sm">{pool.yieldSanity.liquidityScore.toFixed(0)} / 100</p>
              </div>
              <div className="rounded-lg bg-white/[0.04] p-3">
                <p className="text-xs text-white/45">24h Volume</p>
                <p className="mt-1 font-mono text-sm">{usd.format(pool.volume24hUsd)}</p>
              </div>
            </div>
          </article>
        ))}
  ...
  <div>
    <div className="mb-4">
      <h2 className="text-xl font-semibold text-amber-400">Top 5 Lowest Opportunity Pools</h2>
      <p className="mt-1 text-sm text-white/50">Pools with the lowest ratings or yield warnings.</p>
    </div>
    ...
    {poolsLoading ? (
      ...
      <div className="space-y-4">
        {topWorstPools.map((pool, idx) => (
          <article key={pool.address} className="rounded-xl border border-amber-500/20 bg-white/[0.025] p-5">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs text-amber-400">#{idx + 1}</span>
                <div>
                  <h3 className="font-semibold">{pool.token0Symbol} / {pool.token1Symbol}</h3>
                  <p className="mt-1 text-sm text-white/50">TVL {usd.format(pool.tvl)} · Fee APR {pool.feeApr.toFixed(2)}%</p>
                </div>
